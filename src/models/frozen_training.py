...
def weighted_scale_pos_weight(
    ...
    *,
    ...
    if y_array.ndim != 1 or weight_array.ndim != 1:
        raise ValueError(
            "y and sample_weight must be one-dimensional."
        )
    if len(y_array) == 0 or len(y_array) != len(weight_array):
        raise ValueError(
            "y and sample_weight must have equal nonzero length."
        )
    if (
    ...
    if positive_mass <= 0.0 or negative_mass <= 0.0:
        raise ValueError(
            "Both weighted classes are required."
        )
...
def dataframe_fingerprint(
    ...
    if missing:
        raise KeyError(
            f"Fingerprint columns are missing: {missing}"
        )
...
def fitted_feature_names(
    pipeline: Pipeline,
) -> list[str]:
    if "preprocess" not in pipeline.named_steps:
        raise KeyError(
            "Pipeline does not contain a preprocess step."
        )
    return [
        str(value)
        for value in pipeline.named_steps[
            "preprocess"
...
def save_pipeline_with_reload_check(
    ...
    *,
    ...
    probability_before = pipeline.predict_proba(
        X_probe
    )[:, 1]
    joblib.dump(
        pipeline,
    ...
    probability_after = loaded.predict_proba(
        X_probe
    )[:, 1]

    if probability_before.shape != probability_after.shape:
        raise AssertionError(
            "Reloaded model changed prediction shape."
        )
    ...
    if not np.allclose(
        ...
        raise AssertionError(
            "Reloaded pipeline probabilities changed."
        )
    ...
    return {
        ...
        "reload_probability_max_abs_difference": (
